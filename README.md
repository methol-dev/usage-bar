...
Claude's and Codex's subscription usage (5-hour / weekly windows) can be read from **your own signed-in browser sessions** (claude.ai / chatgpt.com) via a companion Chrome extension — a second **data source** for each provider, alongside the CLI/API source. The request is made by your own browser in your real session; **cookies and tokens never leave the browser**, and the extension holds no credentials.
...
4. In UsageBar → **Settings → Providers → (Claude or Codex) → Sources**, enable the **Web** source (and set it as preferred if you want it to win over the CLI/API source).
5. Keep a `claude.ai` / `chatgpt.com` tab signed in — the extension syncs automatically (no manual clicking needed).
...
> The claude.ai / chatgpt.com usage endpoints are undocumented web APIs (no stability guarantee). See [ADR 0009](./docs/adr/0009-claude-web-usage-source.md), [ADR 0010](./docs/adr/0010-claude-multi-source.md), and [ADR 0012](./docs/adr/0012-codex-web-and-generalized-multi-source.md). The extension source lives in [`extension/`](./extension/).
